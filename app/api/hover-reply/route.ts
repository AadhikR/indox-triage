import {
  buildHoverReplyRequest,
  localHoverReply,
  normalizeHoverReply,
} from "@/lib/hover-reply";
import { parseHoverInput } from "@/lib/hover-triage";
...
export async function POST(request: Request) {
  ...
  if (!apiKey) {
    return Response.json(localHoverReply(), { headers: responseHeaders(origin) });
  }
  ...
  try {
    const openRouterResponse = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      headers: {
        ...
        "X-Title": "Inbox Triage Quick Reply",
      },
      body: JSON.stringify(buildHoverReplyRequest(input, model)),
    ...
    if (typeof content !== "string") throw new Error("OpenRouter returned no quick reply.");

    return Response.json(
      { ...normalizeHoverReply(JSON.parse(content)), source: "ai" },
      { headers: responseHeaders(origin) },
    );
  } catch (error) {
    console.error("Quick reply failed; using local fallback.", error);
    return Response.json(localHoverReply(), { headers: responseHeaders(origin) });
  }
}
